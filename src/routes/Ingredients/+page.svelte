...
<section class="space-y-6">
	<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
		<div class="min-w-0 flex-1">
			<h1 class="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-white">Ingredients catalog</h1>
		...
		<div class="flex w-full flex-col gap-2 sm:flex-row sm:items-center sm:justify-end lg:w-auto">
			...
			<input
				...
				class="w-full min-w-[200px] rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm shadow-sm outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/15 sm:max-w-xs dark:border-zinc-600 dark:bg-zinc-900 dark:text-white"
			/>
			<button
				type="button"
					...
					{#each filtered as row (row.id)}
						{#if editingId === row.id}
							<tr class="bg-emerald-50/50 dark:bg-emerald-950/30">
								...
								<td class="px-3 py-2 align-top text-right tabular-nums font-medium dark:text-emerald-300">
									₱{computeUnitCost(
								...
								<td class="px-3 py-2 align-top text-right">
									<button
										type="button"
										class="mr-2 text-xs font-medium text-emerald-700 hover:underline dark:text-emerald-400"
										onclick={saveEdit}
									>
										Save
									</button>
