<script lang="ts">
	...
	let stockRow = $state<OtherItemMasterDTO | null>(null);
	let detailOpen = $state(false);
	let detailRow = $state<OtherItemMasterDTO | null>(null);

	$effect(() => {
		if (typeof localStorage === 'undefined') return;
		const v = localStorage.getItem('pricewise_oth_view');
		if (v === 'cards' || v === 'table') localViewMode = v;
	});

	$effect(() => {
		if (typeof localStorage === 'undefined') return;
		localStorage.setItem('pricewise_oth_view', localViewMode);
	});

	function openStockModal(row: OtherItemMasterDTO, mode: 'receive' | 'use' = 'receive'): void {
		stockRow = row;
		stockModalMode = mode;
		stockModalOpen = true;
	}

	function openDetail(row: OtherItemMasterDTO): void {
		detailRow = row;
		detailOpen = true;
	}
	...
	function handleCardAddStock(row: OtherItemMasterDTO, ev: MouseEvent): void {
		ev.stopPropagation();
	...
	function handleUseStock(packagesQty: number): void {
		if (!stockRow) return;
		useOtherStock(stockRow.id, packagesQty);
	}
		...
		{#if localViewMode === 'cards'}
			{#if otherCatalog.items.length === 0}
				<div class="glass flex flex-col items-center justify-center rounded-3xl py-20 text-center shadow-xl">
					...
					<button onclick={() => (addModalOpen = true)} class="mt-6 rounded-xl bg-zinc-900 px-6 py-2.5 text-sm font-bold text-white">Add Your First Item</button>
				</div>
			...
			{:else}
				<CatalogLocalCards
					rows={filtered}
					accent="sky"
					onSelect={(row) => openDetail(row as OtherItemMasterDTO)}
					onAddStock={(row, ev) => handleCardAddStock(row as OtherItemMasterDTO, ev)}
				/>
			{/if}
		{:else}
				...
				<table class="w-full table-fixed border-collapse text-left text-xs sm:text-sm">
					...
					<tbody class="divide-y divide-zinc-100/50">
						{#each filtered as row (row.id)}
							...
							{:else}
								<tr
									class="group cursor-pointer transition-colors hover:bg-sky-50/40"
